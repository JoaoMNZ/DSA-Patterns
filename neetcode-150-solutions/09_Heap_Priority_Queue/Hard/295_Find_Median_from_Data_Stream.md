...
class MedianFinder {
    ...
    public MedianFinder() {
        // Stores the smaller half, reverse order lets us see the biggest of smalls
        maxHeap = new PriorityQueue<>(Collections.reverseOrder());
        // Stores the larger half, natural order lets us see the smallest of bigs
        minHeap = new PriorityQueue<>();
    }
    
    public void addNum(int num) {
        if(this.size() % 2 == 0){
            // Even size: we want to increase maxHeap size.
            // Check if num belongs in minHeap side
            ...
            // Odd size: maxHeap has 1 more. We want to balance by adding to minHeap.
            // Check if num belongs in maxHeap side
