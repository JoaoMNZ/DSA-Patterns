...
class Twitter {
    ...
    // Helper to ensure user exists in maps
    ...
    public void postTweet(int userId, int tweetId) {
        ...
        userTweets.addLast(new int[]{tweetId, time}); // Add new tweet
        time++;

        // Optimization: Keep only the recent 10 tweets
    ...
    public void unfollow(int followerId, int followeeId) {
        createUser(followerId);
        // Ensure followee exists to avoid NullPointerException
        if(followMap.containsKey(followeeId)){
            // Only need to access follower's list
        }
    ...
    public List<Integer> getNewsFeed(int userId) {
        ...
        // Min-Heap ordered by time (index 1)
        Queue<int[]> minHeap = new PriorityQueue<>(Comparator.comparingInt(t -> t[1]));
        Set<Integer> userFollows = followMap.get(userId);
        
        // Add self to the set of people to check (temporarily or logically)
        // Note: Copying the set or iterating manually prevents concurrent modification issues
        // if we were to modify 'userFollows'. Since we don't modify it here, it is safe.
        List<Integer> peopleToCheck = new ArrayList<>(userFollows);
        peopleToCheck.add(userId);

        for(int followeeId : peopleToCheck){
            Deque<int[]> followeeTweets = tweetMap.get(followeeId);
            if(followeeTweets != null){
                for(int[] tweet : followeeTweets){
                    minHeap.offer(tweet);
                    if(minHeap.size() > 10){
                        minHeap.poll(); // Remove oldest
                    }
                }
            }
        }
        ...
        while(!minHeap.isEmpty()){
            feed.addFirst(minHeap.poll()[0]); // Add to front to reverse order (Newest first)
        }
